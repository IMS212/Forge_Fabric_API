...
public class TerrainRenderContext extends AbstractBlockRenderContext {
	...
	public void tessellateBlock(BlockState blockState, BlockPos blockPos, final BakedModel model, MatrixStack matrixStack, ModelData modelData, RenderLayer renderLayer) {
		try {
			...
			TriState ao = model.useAmbientOcclusion(blockState, modelData, renderLayer);

			aoCalc.clear();
			blockInfo.prepareForBlock(blockState, blockPos, ao != TriState.DEFAULT ? ao == TriState.TRUE : model.useAmbientOcclusion(), modelData, renderLayer);
	...
	public RenderLayer getRenderLayer() {
		return this.blockInfo.renderLayer;
	}

	@Override
	public ModelData getModelData() {
		return this.blockInfo.modelData;
	}
}
